...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([visitApi.stats(), visitApi.myVisits(undefined)])
      .then(([s, v]) => { setStats(s.data); setVisits(v.data.slice(0, 5)); })
      .catch(() => {})
  ...
  return (
    <div className="animate-fade-in">
      ...
      {/* ── Stat cards ── */}
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 lg:gap-4 mb-4 lg:mb-8">
        {statCards.map(({ label, value, icon: Icon, color }) => (
          <div key={label} className="stat-card animate-slide-up">
            <div className="flex items-start justify-between">
              <div className={`w-8 h-8 lg:w-10 lg:h-10 rounded-lg lg:rounded-xl flex items-center justify-center ${colorMap[color]}`}>
                <Icon className="w-4 h-4 lg:w-5 lg:h-5" />
              </div>
              <TrendingUp className="w-3.5 h-3.5 text-gray-300 hidden lg:block" />
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 lg:gap-6">
        ...
        <div className="xl:col-span-2 card">
          <div className="flex items-center justify-between mb-3 lg:mb-5">
            <h2 className="font-bold text-gray-900 lg:text-lg">Recent Visits</h2>
            <Link href="/dashboard/visits"
              className="text-xs text-crimson-600 font-semibold hover:text-crimson-700 flex items-center gap-1">
              View all <ArrowRight className="w-3 h-3 lg:w-3.5 lg:h-3.5" />
            </Link>
          </div>
          ...
          {loading ? (
            <div className="space-y-2 lg:space-y-3">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="h-12 lg:h-16 bg-gray-50 rounded-lg lg:rounded-xl animate-pulse" />
              ))}
            </div>
          ) : visits.length === 0 ? (
            <div className="text-center py-8 lg:py-12 text-gray-400">
              <Users className="w-8 h-8 lg:w-10 lg:h-10 mx-auto mb-2 lg:mb-3 opacity-30" />
            ...
            <div className="space-y-1 lg:space-y-2">
              {visits.map((v) => (
                <div key={v.visit_id}
                  className="flex items-center gap-3 lg:gap-4 p-2 lg:p-3 rounded-lg lg:rounded-xl hover:bg-gray-50 transition-colors">
                  {v.visitor_thumbnail ? (
                    <img src={`data:image/jpeg;base64,${v.visitor_thumbnail}`}
                      className="w-8 h-8 lg:w-10 lg:h-10 rounded-full object-cover border border-gray-200 flex-shrink-0" alt="" />
                  ) : (
                    <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-full bg-crimson-100 flex items-center justify-center flex-shrink-0 text-crimson-600 font-bold text-xs lg:text-sm">
        ...
        {/* Quick actions */}
        <div className="card">
          <h2 className="font-bold text-gray-900 lg:text-lg mb-3 lg:mb-5">Quick Actions</h2>
          <div className="flex flex-col gap-2 lg:gap-2.5">
            <Link href="/dashboard/notifications"
              className="flex items-center gap-3 p-3 lg:p-3.5 rounded-lg lg:rounded-xl border border-crimson-100 bg-crimson-50 hover:bg-crimson-100 transition-colors group">
              <Clock className="w-4 h-4 lg:w-5 lg:h-5 text-crimson-600 flex-shrink-0" />
            ...
            <Link href="/dashboard/search"
              className="flex items-center gap-3 p-3 lg:p-3.5 rounded-lg lg:rounded-xl border border-gray-100 bg-gray-50 hover:bg-gray-100 transition-colors group">
              <Users className="w-4 h-4 lg:w-5 lg:h-5 text-gray-500 flex-shrink-0" />
              <div className="min-w-0">
            ...
            <Link href="/dashboard/visits"
              className="flex items-center gap-3 p-3 lg:p-3.5 rounded-lg lg:rounded-xl border border-gray-100 bg-gray-50 hover:bg-gray-100 transition-colors group">
              <Calendar className="w-4 h-4 lg:w-5 lg:h-5 text-gray-500 flex-shrink-0" />
              <div className="min-w-0">
          ...
          <div className="mt-4 lg:mt-6 pt-4 lg:pt-5 border-t border-gray-100">
            ...
            <div className="space-y-1.5 lg:space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-500 text-xs lg:text-sm">Employee ID</span>
                <span className="font-mono font-semibold text-gray-900 text-xs lg:text-sm">{employee?.employee_id}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500 text-xs lg:text-sm">Department</span>
                <span className="font-semibold text-gray-900 text-xs lg:text-sm">{employee?.department || '—'}</span>
              </div>
            </div>
          </div>
        </div>
